<section class="w-full">
    <x-page-header title="Gestión de Servicios" subtitle="Listado completo de servicios médicos" color="blue">
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-lg rounded-xl overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                    ...
                    <div>
                        <flux:button variant="primary" :href="route('servicios.create')"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <button
                                                ...
                                                type="button" wire:click="delete({{ $servicio->id }})"
                ...
                <div class="block md:hidden space-y-4 px-2 py-4">
                    @forelse ($servicios as $servicio)
                        <div class="bg-white shadow rounded-lg p-4 flex flex-row items-start justify-between gap-2">
                            <div class="flex-1">
                                <div class="font-bold text-blue-700 mb-1">{{ $servicio->nombre }}</div>
                                <div class="text-xs text-gray-500 mb-2 line-clamp-2">{{ $servicio->descripcion }}
                                </div>
                                <div class="text-sm text-gray-700 mb-1">
                                    <strong>Precio:</strong> {{ number_format($servicio->precio, 2) }} €
                                </div>
                                <div class="text-xs text-gray-500 mb-1">
                                    <strong>IVA:</strong> {{ $servicio->iva }}%
                                </div>
                                <div class="mb-1">
                                    @if ($servicio->estado == 'activo')
                                        <span
                            ...
                            <div class="flex flex-col gap-1 items-end">
                                ...
                                <button
                                    class="text-red-600 hover:text-red-900 flex items-center gap-1 px-2 py-1 rounded hover:bg-red-50"
                                    type="button" wire:click="delete({{ $servicio->id }})"
                                    wire:confirm="¿Estás seguro de eliminar este servicio?" title="Eliminar">
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                    ...
                    @empty
                        <div class="text-center text-gray-500">No se encontraron servicios</div>
                    @endforelse
                    <div class="mt-4">
                        {{ $servicios->withQueryString()->links() }}
                    </div>
