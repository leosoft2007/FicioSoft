<!-- filepath: c:\xampp\htdocs\lara12\resources\views\livewire\paciente\form.blade.php -->
<div x-data="{ activeTab: 'informacion' }" class="space-y-6">
    ...
    <div class="border-b border-gray-200 overflow-x-auto">
        <nav class="-mb-px flex min-w-max space-x-4" aria-label="Tabs">
            <button type="button" @click="activeTab = 'informacion'"
                :class="{ 'border-green-500 text-green-600': activeTab === 'informacion', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'informacion' }"
                class="whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm">
                Información Personal
            </button>
            <button type="button" @click="activeTab = 'direccion'"
                :class="{ 'border-green-500 text-green-600': activeTab === 'direccion', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'direccion' }"
                class="whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm">
                Dirección
            </button>
            <button type="button" @click="activeTab = 'emergencia'"
                :class="{ 'border-green-500 text-green-600': activeTab === 'emergencia', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'emergencia' }"
                class="whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm">
                Contacto de Emergencia
            </button>
            <button type="button" @click="activeTab = 'medica'"
                :class="{ 'border-green-500 text-green-600': activeTab === 'medica', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'medica' }"
                class="whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm">
                Información Médica
            </button>
            <button type="button" @click="activeTab = 'estado'"
                :class="{ 'border-green-500 text-green-600': activeTab === 'estado', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'estado' }"
                class="whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm">
                Estado del Paciente
            </button>
            <button type="button" @click="activeTab = 'foto'"
                :class="{ 'border-green-500 text-green-600': activeTab === 'foto', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'foto' }"
                class="whitespace-nowrap py-4 px-2 border-b-2 font-medium text-sm">
                Foto
            </button>
        </nav>
    ...
    <div x-show="activeTab === 'foto'" class="space-y-6">
        <div class="bg-white rounded-lg shadow-sm p-4 border flex flex-col items-center">
          <!--  <flux:heading size="lg">Foto</flux:heading>
            @if ($paciente->foto)
                <img src="{{ asset('storage/' . $paciente->foto) }}" class="w-32 h-32 rounded-full object-cover border mb-4" />
            @endif
            <div x-data="{ preview: null }" class="space-y-4 w-full">
                <flux:input type="file" wire:model="foto" accept="image/*" capture="environment"
                    @change="
                ...
                <template x-if="preview">
                    <img :src="preview" class="w-32 h-32 rounded-full object-cover border mx-auto" />
                </template>
                @error('form.foto')
    ...
    <!-- Acciones -->
    <div class="pt-6 flex flex-col md:flex-row justify-end gap-2">
