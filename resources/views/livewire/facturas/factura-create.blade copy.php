<div>
    <x-page-header title="🧾 {{ $titulo }}" subtitle="" color="green" :clickable="true" badge="Nuevo"
        icon="check" footer="Texto de pie">
                ...
                @if ($recibo_id)
                    ...
                    @if ($recibo && $totalFactura > $recibo->valor)
                        <div class="mt-2 text-red-600 text-sm">
                            ⚠️ El monto de la factura (€{{ number_format($totalFactura, 2) }}) excede el valor del
                            recibo
...
<div class="border-t pt-4">
    ...
    <div class="flex items-end gap-4 mt-2">
            ...
            <flux:select label="➕ Agregar Servicios" wire:model.live="servicio_id">
                ...
                @foreach ($serviciosDisponibles as $servicio)
                    <option value="{{ $servicio->id }}">{{ $servicio->nombre }} -
                        €{{ number_format($servicio->precio, 2) }}</option>
                @endforeach
            </flux:select>
        </div>
        <div class="w-32">
        ...
        <button type="button" wire:click="addServicio"
            class="px-4 py-2 bg-blue-600 text-white rounded-md shadow hover:bg-blue-700">Agregar</button>
    </div>
</div>
...
<div class="mt-4">
    <table class="min-w-full border bg-white border-gray-200 text-sm">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="5" class="text-center py-4 text-gray-500">No hay servicios agregados</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
<div class="flex justify-between items-center mt-6 border-t pt-4">
    <div>
        ...
        <flux:textarea label="Descripción (opcional)" wire:model="descripcion" cols="40" rows="2">
        </flux:textarea>
    </div>
    <div class="text-right">
        <p class="text-lg font-semibold text-gray-800">Total: €{{ number_format($this->calcularTotal(), 2) }}</p>
        <button wire:click="save" class="mt-2 px-5 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">💾
