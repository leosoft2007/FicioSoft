...
<body>
    ...
    <div class="firma-container">
        ...
        <div class="firma-svg-container">
            @if(Str::startsWith($firma, '<svg'))
                {!! $firma !!}
            @elseif(Str::startsWith($firma, 'data:image'))
                <img src="{{ $firma }}" alt="Firma del paciente" style="height: 150px;">
            @else
                <p style="color:red;">Firma no disponible</p>
            @endif
        </div>
    ...
    <div class="metadata">
        ...
        <p><strong>Hash del documento:</strong> {{ $documento_hash }}</p>

        <div class="dispositivo-info">
            <p><strong>Dispositivo:</strong> {{ $dispositivo['user_agent'] ?? 'N/A' }}</p>
            <p><strong>IP:</strong> {{ $dispositivo['ip'] ?? 'N/A' }}</p>
            <p><strong>Fecha del dispositivo:</strong> {{ $dispositivo['fecha'] ?? $fecha }}</p>
        </div>
    </div>
